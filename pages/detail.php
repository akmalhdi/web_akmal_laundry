<?php
$price = 0;
$customers = mysqli_query($config, "SELECT * FROM customers");

$queryService = mysqli_query($config, "SELECT * FROM services");
$rowService = mysqli_fetch_all($queryService, MYSQLI_ASSOC);

$id_user = $_GET['detail'];
$selectDetail = mysqli_query($config, "SELECT * FROM trans_orders WHERE id = $id_user");
$rowDetail = mysqli_fetch_assoc($selectDetail);
if (isset($_GET['detail'])) {
  // print_r($id_user); die;
  $id_order = $rowDetail['id'];
  $selectService = mysqli_query($config, "SELECT trans_order_details.*, services.name FROM trans_order_details 
    LEFT JOIN services ON services.id = trans_order_details.id_service 
      ...
      <form method="POST">
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <input type="date" name="order_date" class="form-control"
              value="<?= date('Y-m-d', strtotime($rowDetail['created_at'])) ?>" readonly>
          </div>
        </div>
        ...
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <select name="order_status" class="form-control" required <?php if (isset($_GET['detail'])) echo 'disabled'; ?>>
              <option value="0" <?= (isset($rowDetail) && ((int)$rowDetail['order_status'] === 1)) ? 'selected' : '' ?>>On Process</option>
              <option value="1" <?= (isset($rowDetail) && ((int)$rowDetail['order_status'] === 0)) ? 'selected' : '' ?>>Done</option>
            </select>
        ...
        <div id="container">
          ...
          <div class="d-flex align-items-center justify-content-end mb-2">
            <label class="form-label me-2 mb-0">Tax (<?= $rowDetail['active_tax'] ?>)%</label>
            <input type="text" class="form-control form-control" style="width: 250px;" readonly value="<?= "Rp " . number_format($rowDetail['order_tax'], 0, ',', '.') ?>">
          </div>
          <div class="d-flex align-items-center justify-content-end mb-2">
            <label class="form-label me-2 mb-0">Total</label>
            <input type="text" class="form-control form-control" style="width: 250px;" readonly value="<?= "Rp " . number_format($rowDetail['order_total'], 0, ',', '.') ?>">
          </div>
          <?php if (isset($rowDetail['order_status']) && $rowDetail['order_status'] == 0): ?>
            <div class="d-flex align-items-center justify-content-end mb-2">
              <label class="form-label me-2 mb-0">Payment</label>
              <input type="text" class="form-control form-control" style="width: 250px;" readonly value="<?= "Rp " . number_format($rowDetail['order_pay'], 0, ',', '.') ?>">
            </div>
            <div class="d-flex align-items-center justify-content-end mb-2">
              <label class="form-label me-2 mb-0">Change</label>
              <input type="text" class="form-control form-control" style="width: 250px;" readonly value="<?= "Rp " . number_format($rowDetail['order_change'], 0, ',', '.') ?>">
